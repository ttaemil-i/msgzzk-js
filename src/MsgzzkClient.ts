...
export class MsgzzkClient extends EventEmitter {
    ...
    public async connect(): Promise<void> {
        ...
        try {
            const { chatChannelId, chatServerId, accessToken } = await this.getChatInfo()
            this.cid = chatChannelId
            const wsUrl = `wss://${chatServerId}/chat`
            this.emit('log', `connecting to ${wsUrl} ...`)
            this.ws = new WebSocket(wsUrl, { headers: this.config.api.headers })

            this.ws.on('open', () => {
                this.emit('log', 'webSocket connected. sending auth (cmd 100)...')
    ...
    private async getChatInfo(): Promise<{ chatChannelId: string, chatServerId: string, accessToken: string }> {
        ...
        const chatServerId = content.chatServerId || 'kr-ss4.chat.naver.com'
        const accessToken = content.chatActive ? content.chatSessionId : ''
        this.emit('log', `cid acquired: ${content.chatChannelId}, chatServerId: ${chatServerId}`)
        return { chatChannelId: content.chatChannelId, chatServerId, accessToken }
    }
        ...
        try {
            ...
            switch (cmd) {
                ...
                case this.config.websocket.commands.historyResponse:
                    this.emit('log', 'received historical messages, ignoring them as requested.')
                    break
                default:
    ...
    private sendPong() {
        ...
        this.ws.send(JSON.stringify({ ver: "3", cmd: cmd }))
    ...
    private handleMessageList(messages: any[] | any, type: 'history' | 'realtime' | 'donation') {
        ...
        list.forEach((rawMsg: any) => {
            ...
            try { profile = rawMsg.profile ? JSON.parse(rawMsg.profile) : {} } catch { profile = {} }
            try { extras = rawMsg.extras ? JSON.parse(rawMsg.extras) : {} } catch { extras = {} }
            ...
            if (msgTypeCode === SUBSCRIBE_CODE) {
                finalMessageType = 'subscribe'
            } else if (type === 'donation' || (msgTypeCode && msgTypeCode !== 1)) {
                finalMessageType = 'donation'
            } else {
                finalMessageType = 'chat'
            }
