...
export class MsgzzkClient extends EventEmitter {
    ...
    public async connect(): Promise<void> {
        ...
        try {
            ...
            this.ws = new WebSocket(this.config.websocket.url, { headers: this.config.api.headers });
    ...
    private async getChatInfo(): Promise<{ chatChannelId: string, accessToken: string }> {
        ...
        return { chatChannelId: content.chatChannelId, accessToken: accessToken };
    }
    ...
    private sendAuth(accTkn: string) {
        ...
        const authPayload = this.createBaseBody(cmd, {
            uid: null, devType: defaults.devType, accTkn: accTkn, auth: defaults.auth,
        });
    ...
    private handleMessageList(messages: any[] | any, type: 'history' | 'realtime' | 'donation') {
        ...
        list.forEach((rawMsg: any) => {
            ...
            try {
                profile = rawMsg.profile ? JSON.parse(rawMsg.profile) : {};
            } catch { profile = {}; }

            try {
                extras = rawMsg.extras ? JSON.parse(rawMsg.extras) : {};
            } catch { extras = {}; }
